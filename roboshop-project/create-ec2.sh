...
if [ -z "${PRIVATE_IP}" ]; then
  ...
  aws ec2 run-instances --image-id ${AMI_ID} --instance-type t3.micro --output text --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${INSTANCE_NAME}}]" --instance-market-options "MarketType=spot,SpotOptions={InstanceInterruptionBehavior=stop,SpotInstanceType=persistent}" --security-group-ids "${SG_ID}"
else
...
aws route53 change-resource-record-sets --hosted-zone-id $ZONE_ID.internal --change-batch file:///tmp/record.json --output text
